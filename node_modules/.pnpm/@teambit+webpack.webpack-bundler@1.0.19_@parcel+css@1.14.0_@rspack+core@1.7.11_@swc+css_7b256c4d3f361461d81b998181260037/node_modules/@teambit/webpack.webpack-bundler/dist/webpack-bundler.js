...
class WebpackBundler {
    ...
    getEntriesAssetsMap(stats, assetsMap) {
        ...
        Object.entries(entriesMap).forEach(([, entryVal]) => {
            ...
            // Get missing chunks from `stats.chunks`
            const chunksForEntry = stats.chunks?.filter((chunk) => {
                return chunk.runtime?.includes(entryVal.name);
            });
            for (const chunk of chunksForEntry || []) {
                const { files } = chunk;
                for (const file of files) {
                    if (!entryVal.assets?.some((asset) => asset.name === file)) {
                        const fileChunk = stats.assets?.find((asset) => asset.name === file);
                        if (fileChunk) {
                            entryVal.assets?.push({
                                name: file,
                                size: fileChunk.size,
                            });
                        }
