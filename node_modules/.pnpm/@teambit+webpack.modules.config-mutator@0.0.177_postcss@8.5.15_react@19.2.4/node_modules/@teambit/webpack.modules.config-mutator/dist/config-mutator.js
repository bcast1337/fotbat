...
class WebpackConfigMutator {
    ...
    removeAliases(aliases) {
        ...
        if ((0, lodash_1.isObject)(this.raw?.resolve?.alias)) {
    ...
    addPostCssPlugins(plugins) {
        this.raw.module?.rules?.forEach((rule) => {
            if (rule.use)
    ...
    addElementToHtmlTemplate(element) {
        ...
        const htmlPlugins = this.raw?.plugins?.filter((plugin) => plugin.constructor.name === 'HtmlWebpackPlugin');
        if (htmlPlugins) {
    ...
    removeElementFromHtmlTemplate(element) {
        ...
        const htmlPlugin = this.raw?.plugins?.find((plugin) => plugin.constructor.name === 'HtmlWebpackPlugin');
        if (htmlPlugin) {
            const htmlContent = typeof htmlPlugin.options?.templateContent === 'function'
