...
export function generateExternalsTransformer(depes: string[]) {
  return (config: RspackConfigMutator): RspackConfigMutator => {
...
export function generateAddAliasesFromPeersTransformer(
  peers: string[],
  logger: Logger
) {
  return (
    ...
    const hostRootDir = context.target?.hostRootDir || context.hostRootDir; // TODO ???
    const peerAliases = peers.reduce<Record<string, string>>(
      (acc, peerName) => {
