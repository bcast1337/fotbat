 ...
 * The Odds API client.
 *
 * Fetches real bookmaker odds (1X2 / h2h market) for football competitions,
 * with TTL caching to respect the free-tier monthly quota (500 requests) and
 * avoid duplicate calls. Odds are matched to fixtures by team-name similarity
 * since The Odds API uses its own event ids.
 */

const BASE_URL = 'https://api.the-odds-api.com/v4';

/** The Odds API sport keys for the competitions we track. */
...
/** A raw event from The Odds API (subset of fields used). */
type RawEvent = {
  id: string;
  home_team: string;
  away_team: string;
...
/** A normalized odds entry keyed for fixture matching. */
...
/** Average the 1X2 odds across all bookmakers for fair-market pricing. */
...
/**
 * Fetch and normalize odds for a competition (cached 2h).
 * @param competitionCode - football-data.org code (e.g. "PL").
 * @returns normalized odds entries, or [] if the sport is unsupported.
 */
