...
/**
 * Frontend API client for the Football Intelligence backend.
 */
...
export function backendUrl(): string {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (origin.includes('p3001.workspaces.bit.cloud')) {
      return origin.replace('p3001.', 'p3002.');
    }
    if (origin.includes('localhost:3001')) {
      return origin.replace(':3001', ':3002');
    }
  }
...
export async function fetchPredictions(days = 10): Promise<Prediction[]> {
  const res = await fetch(`${backendUrl()}/api/predictions?days=${days}`, {
    credentials: 'include',
  });
...
export async function fetchStats(days = 10): Promise<Stats> {
  const res = await fetch(`${backendUrl()}/api/stats?days=${days}`, {
    credentials: 'include',
  });
...
export type BacktestSummary = {
  ...
  valueBetHitRate: number;
  valueBetRoi: number;
  competitions: CompetitionBacktest[];
};
...
export async function patchBet(
  ...
  const res = await fetch(`${backendUrl()}/api/bets/${id}`, {
    method: 'PATCH',
    credentials: 'include',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchBankroll(initial = 1000): Promise<BankrollSummary> {
  const res = await fetch(`${backendUrl()}/api/bankroll?initial=${initial}`, {
    credentials: 'include',
  });
